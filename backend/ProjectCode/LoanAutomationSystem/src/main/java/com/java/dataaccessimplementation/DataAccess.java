...
public class DataAccess implements DataAccessInterface {
	...
	public ArrayList<Customer> getAllCustomers() throws Exception {
		...
		ArrayList<Customer> customers = new ArrayList<>();
		try {
			ps = connection.prepareStatement("SELECT CUSTOMER_ID,FIRST_NAME,LAST_NAME,GENDER,ADDRESS,MOBILE FROM CUSTOMERS ORDER BY CUSTOMER_ID");
			ResultSet res = ps.executeQuery();
			while (res.next()) {
				customers.add(new Customer(res.getInt(1), res.getString(2), res.getString(3),res.getString(4),res.getString(5),res.getString(6)));
			}
	...
	public ArrayList<Loan> browseLoans() throws Exception {
		...
		try {
			PreparedStatement ps = connection.prepareStatement("SELECT LOAN_ID,LOAN_TYPE,INTEREST FROM LOANS ORDER BY LOAN_ID");
			ResultSet res = ps.executeQuery();
			while (res.next()) {
	...
	public ArrayList<LoanApplication> getAllApplications() throws Exception {
		...
		try {
			ps = connection.prepareStatement(
					"SELECT APPLICATION_ID,CUSTOMER_ID,LOAN_TYPE,LOAN_AMOUNT,INTEREST,TENURE,EMI,STATUS,APPLY_DATE FROM LOAN_APPLICATION ORDER BY APPLICATION_ID");
			ResultSet res = ps.executeQuery();
			while (res.next()) {
				applications.add(new LoanApplication(res.getInt(1), res.getInt(2), res.getString(3),res.getString(4), res.getInt(5),res.getInt(6),res.getString(7),res.getString(8),res.getString(9)));
			}
	...
	public ArrayList<LoanApplication> getLoanApplications(String loanType) throws Exception {
		...
		try {
			ps = connection.prepareStatement(
					"SELECT APPLICATION_ID,CUSTOMER_ID,LOAN_TYPE,LOAN_AMOUNT,INTEREST,TENURE,EMI,STATUS,APPLY_DATE FROM LOAN_APPLICATION WHERE LOAN_TYPE=? ORDER BY APPLICATION_ID");
	...
	public ArrayList<LoanApplication> getApplicationsByStatus(String status) throws Exception {
		...
		try {
			ps = connection.prepareStatement(
					"SELECT APPLICATION_ID,CUSTOMER_ID,LOAN_TYPE,LOAN_AMOUNT,INTEREST,TENURE,EMI,STATUS,APPLY_DATE FROM LOAN_APPLICATION WHERE STATUS=? ORDER BY APPLICATION_ID");
	...
	public ArrayList<LoanApplication> getApplicationsByCId(int customerId) throws Exception {
		...
		try {
			ps = connection.prepareStatement(
					"SELECT APPLICATION_ID,CUSTOMER_ID,LOAN_TYPE,LOAN_AMOUNT,INTEREST,TENURE,EMI,STATUS,APPLY_DATE FROM LOAN_APPLICATION WHERE CUSTOMER_ID =? ORDER BY APPLICATION_ID");
			ps.setInt(1, customerId);
			ResultSet res = ps.executeQuery();
			while (res.next()) {
				applications.add(new LoanApplication(res.getInt(1), res.getInt(2), res.getString(3),res.getString(4), res.getInt(5),res.getInt(6),res.getString(7),res.getString(8),res.getString(9)));
...
//	}
	...
	public ArrayList<LoanApplication> getApplicationsByDate(String date) throws Exception {
		...
		try {
			ps = connection.prepareStatement(
					"SELECT APPLICATION_ID,CUSTOMER_ID,LOAN_TYPE,LOAN_AMOUNT,INTEREST,TENURE,EMI,STATUS,APPLY_DATE FROM LOAN_APPLICATION WHERE APPLY_DATE =? ORDER BY APPLICATION_ID");
			ps.setString(1, date);
			ResultSet res = ps.executeQuery();
			while (res.next()) {
				applications.add(new LoanApplication(res.getInt(1), res.getInt(2), res.getString(3),res.getString(4), res.getInt(5),res.getInt(6),res.getString(7),res.getString(8),res.getString(9)));
